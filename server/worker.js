...
export default {
  async fetch(request, env) {
    try {
      ...
      // Public sizing calculator (no auth required for guest store visitors)
      if (pathname === "/api/sizing/calculate" && request.method === "POST") {
        const body = await request.json();
        const sizing = calculateProfile(body);
      ...
      if (pathname === "/api/auth/register" && request.method === "POST") {
        ...
        const userRow = await env.DB.prepare("SELECT id FROM users WHERE uuid = ?").bind(uuid).first();
